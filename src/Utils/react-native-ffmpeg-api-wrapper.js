...
export function executeFFmpegWithArguments(commandArguments) {
  RNFFmpeg.executeWithArguments(commandArguments).then(rc =>
    ffprint(`FFmpeg process exited with rc ${rc}`),
  )
}
...
export function executeFFmpegAsyncWithArguments(commandArguments, callback) {
  RNFFmpeg.executeAsyncWithArguments(
    ...
    ffprint(`FFmpeg process started with executionId ${executionId}.`),
  )
}
...
export function executeFFprobeWithArguments(commandArguments) {
  RNFFprobe.executeWithArguments(commandArguments).then(rc =>
    ffprint(`FFprobe process exited with rc ${rc}`),
  )
}
